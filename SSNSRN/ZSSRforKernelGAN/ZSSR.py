...
class ZSSR:
    ...
    def build_network(self, meta):
        with self.model.as_default():
            ...
            for l in range(meta.depth - 1):
                if l>1:
                    self.layers_t[l + 1] = tf.nn.relu(tf.nn.conv2d(self.layers_t[l]+self.layers_t[l-1], self.filters_t[l], [1, 1, 1, 1], "SAME", name='layer_%d' % (l + 1)))
                else:
    ...
    def train(self):
        ...
        for self.iter in range(self.conf.max_iters):
            ...
            if self.iter == 1:
                plt.imsave('./lr.png', self.lr_son, dpi=1)
