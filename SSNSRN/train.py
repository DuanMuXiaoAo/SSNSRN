import os
import tqdm

from configs import Config
from data import DataGenerator
from kernelGAN import KernelGAN
from learner import Learner


def train(conf):
    gan = KernelGAN(conf)
    learner = Learner()
    data = DataGenerator(conf, gan)
    for iteration in tqdm.tqdm(range(conf.max_iters), ncols=60):
        [g_in, d_in] = data.__getitem__(iteration)
        gan.train(g_in, d_in)
        learner.update(iteration, gan)
    gan.finish()


def main():
    """训练主要流程：进行模糊核的计算及超分辨（可选）"""
    import argparse
    # 解析命令行参数，不得不说argparse真好用(*^▽^*)
    prog = argparse.ArgumentParser()
    prog.add_argument('--input-dir', '-i', type=str, default='test_images', help='输入图像文件夹路径')
    prog.add_argument('--output-dir', '-o', type=str, default='results', help='结果保存文件夹路径')
    prog.add_argument('--X4', action='store_true', help='设定放大倍数为4倍，默认为2倍')
    prog.add_argument('--SR', action='store_true', help='开启超分辨部分')
    prog.add_argument('--real', action='store_true', help='ZSSR部分计算的是real-image，这里没搞懂，不过影响不大，不用这个')
    prog.add_argument('--noise_scale', type=float, default=1., help='ZSSR部分设定，去噪用')
    args = prog.parse_args()
    # Run the KernelGAN sequentially on all images in the input directory
    for filename in os.listdir(os.path.abspath(args.input_dir)):
        conf = Config().parse(create_params(filename, args))
        train(conf)
        # os.remove(conf.input_image_path)
    prog.exit(0)


def create_params(filename, args):
    params = ['--input_image_path', os.path.join(args.input_dir, filename),
              '--output_dir_path', os.path.abspath(args.output_dir),
              '--noise_scale', str(args.noise_scale)]
    if args.X4:
        params.append('--X4')
    if args.SR:
        params.append('--do_ZSSR')
    if args.real:
        params.append('--real_image')
    return params


if __name__ == '__main__':
    main()
